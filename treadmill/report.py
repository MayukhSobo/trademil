...
def create_training_summary_table(report: TrainingReport) -> Table:
    ...
    table = Table(title="📊 Training Summary", box=box.ROUNDED, show_header=True, header_style="bold magenta")
    table.add_column("Metric", style="cyan", no_wrap=True, width=25)
    table.add_column("Value", style="green", width=30)
    
    # Training details
    table.add_row("Total Epochs", str(report.total_epochs))
    table.add_row("Training/Resume Time", format_time_duration(report.training_time))
    ...
    if report.early_stopped:
        table.add_row("Early Stopping", f"✅ Yes (Epoch {report.early_stop_epoch})", style="yellow")
    else:
...
def create_model_info_table(report: TrainingReport) -> Table:
    ...
    table = Table(title="🏗️ Model Information", box=box.ROUNDED, show_header=True, header_style="bold magenta")
...
def create_hardware_info_table(report: TrainingReport) -> Table:
    ...
    table = Table(title="💻 Hardware Resources", box=box.ROUNDED, show_header=True, header_style="bold magenta")
    ...
    if report.avg_cpu_percent is not None:
        table.add_row(
            "CPU Usage (%)",
            f"{report.avg_cpu_percent:.1f}%" if report.avg_cpu_percent is not None else "N/A",
            f"{report.max_cpu_percent:.1f}%" if report.max_cpu_percent is not None else "N/A"
        )
    ...
    if report.avg_ram_mb is not None:
        table.add_row(
            "RAM Usage",
            format_memory(report.avg_ram_mb) if report.avg_ram_mb is not None else "N/A",
            format_memory(report.max_ram_mb) if report.max_ram_mb is not None else "N/A"
        )
    ...
    if report.num_gpus > 0:
        ...
        if report.avg_gpu_utilization is not None:
            table.add_row(
                "GPU Utilization (%)",
                f"{report.avg_gpu_utilization:.1f}%" if report.avg_gpu_utilization is not None else "N/A",
                f"{report.max_gpu_utilization:.1f}%" if report.max_gpu_utilization is not None else "N/A"
            )
        ...
        if report.gpu_memory_used_mb is not None and report.gpu_memory_total_mb is not None:
            ...
            table.add_row(
                "GPU Memory",
                f"{format_memory(report.gpu_memory_used_mb)} / {format_memory(report.gpu_memory_total_mb)}",
                f"({usage_percent:.1f}%)"
            )
...
def create_checkpoint_info_table(report: TrainingReport) -> Table:
    ...
    table = Table(title="💾 Checkpoint Information", box=box.ROUNDED, show_header=True, header_style="bold magenta")
    table.add_column("Property", style="cyan", no_wrap=True, width=25)
...
def create_training_report_from_trainer(trainer) -> TrainingReport:
    ...
    if hasattr(trainer, 'config'):
        ...
        report.optimizer = config.optimizer.optimizer_class.__name__ if hasattr(config.optimizer, 'optimizer_class') else "Unknown"
        if config.scheduler:
            report.scheduler = config.scheduler.scheduler_class.__name__ if hasattr(config.scheduler, 'scheduler_class') else "Unknown"
